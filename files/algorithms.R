...
GKCM_KRR_wrapper <- function(data, job, instance, ...){
  ...
  gaussian_kernel <- gaussian_kernel <- function(X, sigma) {
  ...
  GKCM_KRR <- function(X, Y, Z, lambda = 0.001, sigma = 1, 
                       seed = NULL, d = NULL) {
    ...
    M_X <- gaussian_kernel(Z, med_heur)
    M_Y <- gaussian_kernel(Z, med_heur)
...
RCIT_wrapper <- function(data, job, instance, ...){
  ...
  RCIT <- function(X, Y, Z, lambda = 0.001, sigma = 1,  
    ...
    XZ_RFF <- RFF(cbind(X,Z), n_RFF_xy, 
                 sigma = c(sigma, rep(2*med_heur, ncol(Z)))) |> scale()
    Y_RFF <- RFF(Y, n_RFF_xy, sigma = sigma) |> scale()
    Z_RFF <- RFF(Z, n_RFF_z, sigma = med_heur) |> scale()
...
RCoT_wrapper <- function(data, job, instance, ...){
  ...
  RCoT <- function(X, Y, Z, lambda = 0.001, sigma = 1, 
                   n_RFF_xy = 10, n_RFF_z = 200, seed = NULL){
  ...
  X_RFF <- RFF(X, n_RFF_xy, sigma = sigma) |> scale()
  Y_RFF <- RFF(Y, n_RFF_xy, sigma = sigma) |> scale()
  Z_RFF <- RFF(Z, n_RFF_z, sigma = med_heur) |> scale()
